...
class OrchestratorConfigTests(unittest.TestCase):
    def test_legacy_stage_threshold_keys_are_ignored(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertAlmostEqual(float(loaded.fusion.uncertainty_weight_gamma), 1.5, places=12)
            self.assertAlmostEqual(float(loaded.fusion.weight_floor), 0.1, places=12)
            self.assertAlmostEqual(float(loaded.decision.accuracy_floor_delta), 0.02, places=12)
            self.assertTrue(bool(loaded.decision.defer_policy.enabled))
            self.assertAlmostEqual(float(loaded.decision.defer_policy.uncertainty_threshold), 0.66, places=12)
            self.assertAlmostEqual(float(loaded.decision.defer_policy.margin_from_half), 0.08, places=12)
            self.assertTrue(bool(loaded.decision.defer_policy.require_all_agents_exhausted))
            ...
            self.assertEqual(loaded.benchmark.utility_evaluation, "cost_action_parity")
