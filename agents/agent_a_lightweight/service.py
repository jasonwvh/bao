...
from .agent import LightweightAgent


AGENT = LightweightAgent(cost=float(os.getenv("AGENT_COST", "1.0")))
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == "/a2a/capabilities":
            return self._send({"agent_id": AGENT.agent_id, "capabilities": ["flow_tabular", "baseline"], "cost": AGENT.cost})
        self.send_error(404)
