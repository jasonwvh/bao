...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cost", type=float, default=8.0, help="Cost per inference")
    return p.parse_args()
...
def main() -> None:
    ...
    if model_path is None:
        model_path = os.path.join(os.path.dirname(__file__), "models", "wgan_gp.pt")

    from agents.wgan_gp.service import WGANGPAgent

    agent = WGANGPAgent(model_path=model_path, cost=args.cost)
